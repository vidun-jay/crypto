...
def get_balance(username):
    ...
    for block in blockchain.chain:
        ...
        if username == data[0]: # if the username is in the first position, they are sending money
            balance -= float(data[2])
        elif username == data[1]: # if the username is in the first position, they are recieving money
            balance += float(data[2])
    return balance
